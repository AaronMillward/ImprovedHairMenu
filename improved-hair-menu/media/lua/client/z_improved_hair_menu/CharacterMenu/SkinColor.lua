...
function CharacterCreationMain:createChestTypeBtn()
	...
	-- XXX: Essentially replaces `onSkinColorPicked()`
	function self.skinColorButton.attachedPanel.onSelect(info)
		self.skinColorButton.backgroundColor = { r=info.buttonColor.r, g=info.buttonColor.g, b=info.buttonColor.b, a = 1 }
		local desc = MainScreen.instance.desc
		desc:getHumanVisual():setSkinTextureIndex(info.id - 1)
	...
	self.skinColorButton.attachedPanel:setInfoTable({
		...
		},
		{
			...
			getterName = "getSkinTextureIndex",
			setterName = "setSkinTextureIndex",
			buttonColor = {r=0.98,g=0.79,b=0.49},
		},
		{
			id = 3,
			display = "Medium",
			getterName = "getSkinTextureIndex",
			setterName = "setSkinTextureIndex",
			buttonColor = {r=0.8,g=0.65,b=0.45},
		},
		{
			id = 4,
			display = "Dark",
			getterName = "getSkinTextureIndex",
			setterName = "setSkinTextureIndex",
			buttonColor = {r=0.54,g=0.38,b=0.25},
		},
		{
			id = 5,
			display = "Darkest",
			getterName = "getSkinTextureIndex",
			setterName = "setSkinTextureIndex",
			buttonColor = {r=0.36,g=0.25,b=0.14},
		},
	})

	-- NOTE: We have to leave this old color picker here for saving to work.
...
-- 	self.ICSSkinColorPicker:setX(button:getAbsoluteX())
-- 	self.ICSSkinColorPicker:setY(button:getAbsoluteY() + button:getHeight())
-- 	local color = button.backgroundColor
-- 	self:removeChild(self.ICSSkinColorPicker)
-- 	self:addChild(self.ICSSkinColorPicker)
-- 	if self.characterPanel.joyfocus then
-- 		self.characterPanel.joyfocus.focus = self.ICSSkinColorPicker
-- 	end
-- end